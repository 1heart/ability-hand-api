...
from psyonic_ability_hand.hand import Hand, Grip, HandException, JointData, Mode
import pytest
import binascii
import array
...
def test_set_grip_speed(hand):
    with pytest.raises(HandException):
...
def test_set_mode_pos_ctl(hand):
    expected = array.array("B", [0xAD] + [0x00] * 24 + [0x53])
